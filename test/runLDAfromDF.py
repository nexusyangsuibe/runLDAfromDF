...
def runLDAwithRetry(runtime_code,num_topic,text_features,df_identity_code,tokenized_column_name,text_feature_extractor,n_jobs,retry=0):
    ...
    n_jobs=int(n_jobs*0.78) if n_jobs>os.cpu_count()*0.3 else n_jobs-1
    if n_jobs<1:
...
def run_lda_once(runtime_code,num_topic,text_features,df_identity_code,tokenized_column_name,text_feature_extractor,n_jobs):
    ...
    lda=LatentDirichletAllocation(n_components=num_topic,learning_method="batch",max_iter=16,max_doc_update_iter=160,n_jobs=n_jobs) # I want to add an option to use 'online' learning method, but it me encounter some weird mistakes like fail to pickle so I forgive it
    lda_dtf=lda.fit_transform(text_features)
    perplexity=int(lda.bound_)
    pickle.dump(tuple([lda,lda_dtf]),open(f"respawnpoint/{runtime_code}_lda_{num_topic}_{perplexity}_{df_identity_code}_{tokenized_column_name}_{text_feature_extractor}.pkl","wb"))
    return perplexity
...
def runLDAfromDF(runtime_code,input_file,tokenized_column_name,text_feature_extractor,start_num_topic,end_num_topic,num_topic_step,output_filename,clear_respawnpoint_before_run,clear_respawnpoint_upon_conplete):
    ...
    if unfinished_num_topics:
        n_jobs_beg=min(os.cpu_count(),int(os.cpu_count()**2/(unfinished_num_topics[0]*memory_usage_GB*2))) # this formula is just a rule of thumb for personal pc modified for big data research use with about 0.5 cpu_kernel/memory_size. it sounds a good idea to get the cpu kernel amounts and memory size at the same time and use a function(cpu_count,memory_size,file_size) to determine the processors used and the batch size, but it may encounter some problems with authority
        if n_jobs_beg<2:
            n_jobs_beg=2
